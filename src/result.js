          ...
          mbti === answerList[i].m ? (
            <>
              <div
                className="mx-auto"
                onLoad={() => {
                  setSrc("/img/" + answerList[i].id + ".png");
                  setID(answerList[i].id);
                }}
              >
                <img
                  className="mx-auto mt-3 col-lg-3 col-mg-3 col-sm-6"
                  src={imgSrc}
                  alt={answerList[i].m}
                ></img>
              </div>
              ...
              <div className="answerBox">
                <span
                  dangerouslySetInnerHTML={{ __html: answerList[i].a }}
                ></span>
              </div>
              <div className="line my-5"></div>
              {isDesktopOrMobile !== true ? (
                <div className="mt-5" style={{ display: "flex" }}>
                  <div className="matchBox mx-auto">
                    ...
                    <img
                      className="mx-auto my-3 col-lg-5 col-mg-3 col-sm-6"
                      src={"/img/" + answerList[i].good_id + ".png"}
                      alt={answerList[i].good_m}
                    ></img>
                    <h4>{answerList[i].good_m}</h4>
                    <h4>
                      <b>{answerList[i].good}</b>
                    </h4>
                  </div>
                  <div className="matchBox mx-auto">
                    <h4>
                    ...
                    <img
                      className="mx-auto my-3 col-lg-5 col-mg-3 col-sm-6"
                      src={"/img/" + answerList[i].bad_id + ".png"}
                      alt={answerList[i].bad_m}
                    ></img>
                    <h4>{answerList[i].bad_m}</h4>
                    <h4>
                      <b>{answerList[i].bad}</b>
                    </h4>
                  </div>
                </div>
              ) : (
              ...
              <Button
                ...
                onClick={() => navigate("/", { state: { mbti: mbti } })}
              >
                다시하기
              </Button>{" "}
              <div className="mt-4">
  ...
  function shareKakao() {
    ...
    return window.Kakao.Link.sendDefault({
      ...
      content: {
        ...
        imageUrl:
          "https://mbtidrink.netlify.app/img/" + answerList[id].id + ".png",
        link: {
          mobileWebUrl:
            "https://mbtidrink.netlify.app/result/" + answerList[id].m,
        },
      },
      buttons: [
        {
